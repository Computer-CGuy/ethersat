...
def run(c,batch=1,build=None):
    ...
    state = [cpu_backend.buffer_from_pyval(special_dtype(0)),cpu_backend.buffer_from_pyval(special_dtype(16))]
    print("Evalving")
    for x in range(64+8):
        state.append(cpu_backend.buffer_from_pyval(As[x]))
    print("Started")
    out = timed(compiled_computation.execute,state,rate=True,batch=batch)
    print(type(out[0].to_py()))
...
def generator():
    ...
    return [mix_shape]+[newdata]+[seed_init]
...
mix,dataset,seed_init = paramaterize(miner, tup)
